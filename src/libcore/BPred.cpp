...
{
    bpredEnergy->inc();
    int instID = inst->currentID();
    if (counter.find(instID) == counter.end()) {
        counter[instID] = std::vector<int>(2, 0);
    }
  ...
  //   } else {
   ...
   //  }
    return inst->calcNextInstID() == oracleID ? CorrectPrediction : MissPrediction;
     ...
     std::vector<double> accuracies(4, 0.0);
     int firstCorrectSum = 0;
     int secondCorrectSum = 0;
     int thirdCorrectSum = 0;

     int fourthCorrectSum = 0;
    ...
    for (const auto& entry : counter) {
        int id = entry.first;
        const std::vector<int> preds = entry.second;
     //   std::cout << std::left << std::setw(20) << id << std::setw(25) << preds[0] << std::setw(25) << preds[1] << std::endl;
     int complete = preds[0] + preds[1];

      if (complete >=1 && complete <= 19) {
       firstCorrectSum += preds[0];

       firstAllSum += complete; 
        buckets[0]++;
      } else if (complete >= 20 && complete <= 199) {
        secondCorrectSum += preds[0];
        secondAllSum += complete; 
         buckets[1]++;
      } else if (complete >= 200 && complete <= 1999) {
        thirdCorrectSum += preds[0];
        thirdAllSum += complete; 
         buckets[2]++;
      } else if (complete >= 2000) {
        fourthCorrectSum += preds[0];
        fourthAllSum += complete; 
         buckets[3]++;
      } 

      }
      accuracies[0] =(double) firstCorrectSum / firstAllSum;
	accuracies[1] = (double)secondCorrectSum / secondAllSum; 
      accuracies[2] = (double)thirdCorrectSum / thirdAllSum;
      accuracies[3] = (double)fourthCorrectSum / fourthAllSum; 
      std::cout << "Numbers for NotTaken Predictor------------------------" << std::endl; 
      std::cout << "Global variables, for consistency purposes: "; 
//      std::cout << "nMiss: " << nMiss << " nHit: " << nHit << std::endl; 

std::cout << "Numbers of Completed Static Branches------------------------------------" << std::endl;
 std::cout << "1-19: " << buckets[0] << " 20-199: " << buckets[1] << " 200-1999: " << buckets[2] << " 2000+: " << buckets[3] << std::endl;
std::cout << std::endl; 
 std::cout << "Accuracies-------------------" << std::endl;
	std::cout << "1-19: " << accuracies[0] << " 20-199: " << accuracies[1] << " 200-1999: " << accuracies[2] << " 2000+: " << accuracies[3] << std::endl; 
}
...
{
    dump(section);
//    std::cout << std::left << std::setw(20) << "Branch Instruction ID" << std::setw(25) << "Correct Predictions" << std::setw(25) << "Incorrect Predictions" << std::endl;
...
//        buckets[3]++;
 ...
 // std::cout << "1 to 19: " << buckets[0] << "20 to 199: " << buckets[1] << "200 to 1999: " << buckets[2] << "2000+ : " << buckets[3] << std::endl; 
